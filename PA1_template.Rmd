...
```{r}
hist(steps_date$steps, breaks=53, main="Total Steps Taken per Day", xlab="Daily Total Steps", col="blue")
```
...
Make the plot
```{r}
plot(steps_interval$interval, steps_interval$steps, type="l", main = "Average Daily Activity Pattern", xlab="5 min interval", ylab="Average Steps")
```
...
```{r}
library(dplyr)
activity2<- activity2 %>% mutate(weektype=ifelse(weekdays(activity2$date)=="Saturday"|weekdays(activity2$date)=="Sunday", "Weekend", "Weekday"))
...
```{r}
library(lattice)
xyplot(steps~interval|weektype, data=steps_interval, layout=c(1,2), type="l", main="Average Number of Steps Taken per Interval by Day", xlab="Interval", ylab="Average Steps")
```
